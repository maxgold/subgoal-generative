...
D_loss_real = tf.nn.sigmoid_cross_entropy_with_logits(logits=D_logit_real, targets=tf.ones_like(D_logit_real))
D_loss_fake = tf.nn.sigmoid_cross_entropy_with_logits(logits=D_logit_fake, targets=tf.zeros_like(D_logit_fake))
...
D_solver = tf.train.AdamOptimizer(0.001).minimize(D_loss, var_list=tf.contrib.slim.get_variables(scope="discriminator"))
G_solver = tf.train.AdamOptimizer(0.001).minimize(G_loss, var_list=tf.contrib.slim.get_variables(scope="generator"))
